...
//Purpose- To give the user a simple math quiz with timed questions
{
    static bool timeOut_b = false;
    static Timer TimerFunction;

    //Author- Parth Rustagi
    //Purpose- To implement Time limit for each question
    static void TimeOut(object source, ElapsedEventArgs e)
    {
        Console.WriteLine("Ding!!!!! Time Out.......You took too much time");
        timeOut_b = true;
        TimerFunction.Stop();
    }

    //Author- Parth Rustagi
    //Purpose - To ask the user for input their name, then number of questions, then difficulty of the questions and then wait for their response.
    static void Main()
    {
        // store user name
        string myName = "";
    ...
    start:
        ...
        for (nCntr = 0; nCntr < nQuestions; ++nCntr)
        {
            ...
            TimerFunction = new Timer(5000);

            ElapsedEventHandler elapsedEventHandler;

            elapsedEventHandler = new ElapsedEventHandler(TimeOut);

            TimerFunction.Elapsed += elapsedEventHandler;
            ...
            if (nResponse == nAnswer && timeOut_b == false)
            {
            ...
            else if (nResponse == nAnswer && timeOut_b == true)
            {
                Console.BackgroundColor = ConsoleColor.Black;
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine("Your answer is correct but you ran out of time\n");
            }
            else if (nResponse != nAnswer && timeOut_b == true)
            {
                Console.BackgroundColor = ConsoleColor.Black;
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine("Your answer is incorrect and also you ran out of time\n");
            }
            else
            {
                Console.BackgroundColor = ConsoleColor.Black;
